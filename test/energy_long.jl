@testset "energy_long long range" begin
    ...
    E_FFCT_loop = energy_long_loop(qs, poses, L, N_grid, M_mid, k_x, k_y, r_z, phase_x, phase_y, z, sort_z, us_mat, b_l, b_u, rhs, sol, ivsm, H_r, H_c, H_s, uspara, soepara)

    E_FFCT_einsum = energy_long_einsum(qs, poses, L, M_mid, k_x, k_y, k_mat, r_z, phase_x, phase_y, phase_xs, phase_ys, phase_xys, temp_ijlk, temp_ijl, size_dict, z_coef, exp_coef, z, sort_z, us_mat, b_l, b_u, rhs, sol, ivsm, H_r, H_c, H_s, uspara, soepara)
    ...
    @show E_FFCT_loop, E_direct, E_FFCT_einsum
    @test E_FFCT_einsum ≈ E_FFCT_loop
    @test isapprox(E_FFCT_loop, E_direct, atol=1e-4)
end