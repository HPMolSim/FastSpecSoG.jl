@inbounds function gather_Q2D_single(q::T, pos::NTuple{3, T}, L::NTuple{3, T}, k_x::Vector{T}, k_y::Vector{T}, phase_x::Vector{Complex{T}}, phase_y::Vector{Complex{T}}, H_s::Array{Complex{T}, 3}) where{T}
    x, y, z = pos
    L_x, L_y, L_z = L
    ϕ = zero(Complex{T})
...
function gather_Q2D(qs::Vector{T}, poses::Vector{NTuple{3, T}}, L::NTuple{3, T}, k_x::Vector{T}, k_y::Vector{T}, phase_x::Vector{Complex{T}}, phase_y::Vector{Complex{T}}, H_s::Array{Complex{T}, 3}) where{T}
    E = zero(Complex{T})
    for i in 1:length(qs)
        E += gather_Q2D_single(qs[i], poses[i], L, k_x, k_y, phase_x, phase_y, H_s)
    end
    return real(E)
end
...
function gather_thin(qs::Vector{T}, poses::Vector{NTuple{3, T}}, L::NTuple{3, T}, H_s::Array{Complex{T}, 3}, gridinfo::GridInfo{2, T}, cheb_value::Vector{Array{T, 1}}, cheb_coefs::NTuple{2, ChebCoef{T}}) where{T}
    E = zero(Complex{T})
    for i in 1:length(qs)
        E += gather_thin_single(qs[i], poses[i], L[3], H_s, gridinfo, cheb_value, cheb_coefs)
    end
    return real(E) / (2 * L[1] * L[2])
end