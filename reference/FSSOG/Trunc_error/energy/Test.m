...
%                 for p=1:N
%                     if (r(p)>0.01)
%                         phi(p,1)=phi(p,1)+0.5*x(p,4)*x(m,4).*(1./r(p));
%                     end
%                 end
%             end
%             %end
    ...
    for j=-Nmax:Nmax
        source(:,2)=x(:,2)+j*2*L; 
            ...
            for m=1:N
                ...
                for p=1:N
                    if(r(p)>=10)
                        phi_RBSOG(p,1)=phi_RBSOG(p,1)+0.5*x(p,4)*x(m,4).*(1./r(p)-SOG(Bcut,w0,base,sigma,r(p)));%1./r(p);%
                    end
                end
            end
    end
...
err=abs((sum(phi_RBSOG)/8)/(0.028167683235590));
